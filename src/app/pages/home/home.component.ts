...
import { BehaviorSubject, Observable, Subscription, finalize, of, tap } from 'rxjs';
...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  loadRandomSingers(reset: boolean = false): void {
    ...
    this.singersService.getSingers({
      take: 10,
  ...
  onRandomAudiosScroll(): void {
    if (this.isScrolledNearRight(this.randomAudiosContainer.nativeElement) && !this.isLoadingRandomAudios) {
      this.loadRandomAudios();
    }
  }

  onTopAudiosScroll(): void {
    if (this.isScrolledNearRight(this.topAudiosContainer.nativeElement) && !this.isLoadingTopAudios) {
      this.loadTopAudios();
    }
  }

  onRandomSingersScroll(): void {
    if (this.isScrolledNearRight(this.randomSingersContainer.nativeElement) && !this.isLoadingRandomSingers) {
      this.loadRandomSingers();
    }
  }

  onRecentAudiosScroll(): void {
    if (this.isScrolledNearRight(this.recentAudiosContainer.nativeElement) && !this.isLoadingRecentAudios) {
      this.loadRecentAudios();
    }
  }
  ...
  setCurrentAudio(audio: Audio, index: number, section: 'random' | 'top' | 'recent'): void {
